...
the group (but only if it isn’t already a member), delete removes its argument from 
the group (if it was a member), and has returns a Boolean value indicating whether 
...
class Group {
  ...
  delete (item) {
    this.group = this.group.filter((e) => {
      return e !== item
    })
  }
